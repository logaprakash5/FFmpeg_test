        ...
        for(osi = 0; osi < 64; osi += 16){
            ...
            for (fsi = 0; fsi < FILTER_SIZES; ++fsi) {
                ...
                if (check_func(c->yuv2planeX, "yuv2yuvX_%d_%s_%d_%d_%d_%s", bit_depth, isBE(dst_pix_format) ? "BE" : "LE", filter_sizes[fsi], osi, dstW, accurate_str)){
                    ...
                    {
                        call_ref(&filter_coeff[0], filter_sizes[fsi], src, (uint8_t*)dst0, dstW, dither, osi);
                        call_new(&filter_coeff[0], filter_sizes[fsi], src, (uint8_t*)dst1, dstW, dither, osi);
